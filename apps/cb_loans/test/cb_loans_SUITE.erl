...
calculate_monthly_payment_ok(_Config) ->
    {ok, Payment} = cb_loan_calculations:calculate_monthly_payment(10000, 12, 0.12),
    ?assert(Payment > 0),
...
calculate_monthly_payment_zero_rate(_Config) ->
    {ok, Payment} = cb_loan_calculations:calculate_monthly_payment(12000, 12, 0.0),
    ?assertEqual(1000, Payment),
    ok.

%% Happy path: Calculate total interest
calculate_total_interest_ok(_Config) ->
    {ok, TotalInterest} = cb_loan_calculations:calculate_total_interest(10000, 12, 0.12),
...
calculate_interest_portion_ok(_Config) ->
    InterestPortion = cb_loan_calculations:calculate_interest_portion(10000, 1000, 0.12),
    ?assert(InterestPortion > 0),
    ok.

%% Happy path: Calculate flat interest
calculate_flat_interest_ok(_Config) ->
    FlatInterest = cb_loan_calculations:calculate_flat_interest(10000, 12, 0.12),
...
calculate_monthly_payment_invalid_amount(_Config) ->
    {error, Reason} = cb_loan_calculations:calculate_monthly_payment(0, 12, 0.1),
...
calculate_monthly_payment_rate_too_high(_Config) ->
    {error, Reason} = cb_loan_calculations:calculate_monthly_payment(10000, 12, 1.01),
